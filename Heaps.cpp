...
//         // my logic: above one is wrong:
//         if(size == 0) return;
//         arr[1] = arr[size];
//         size--;
//         int index = 1;
//         while(2 * index <= size) { // leftchild is available
//             int leftChildIndex = 2 * index;
//             int rightChildIndex = 2 * index + 1;
//             int swapIndex = leftChildIndex;
//             if(rightChildIndex <= size && arr[rightChildIndex] > arr[leftChildIndex]) swapIndex = rightChildIndex;
...
class Heap {
    ...
    void takeAtCorrPos(int val, int index) {
        if(index > 1) {
            int parentIndex = index / 2;
            if(arr[parentIndex] < val) {
                swap(arr[parentIndex], arr[index]);
                takeAtCorrPos(val, parentIndex);
            }
        }
    }
